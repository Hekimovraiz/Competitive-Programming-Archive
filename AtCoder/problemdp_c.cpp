...
void solve()
{
    ...
    dp[1][1] = v[1][1], dp[1][2] = v[1][2], dp[1][3] = v[1][3];
    for (int i = 2; i <= n; ++i)
    {
        dp[i][1] = v[i][1] + max(dp[i - 1][2], dp[i - 1][3]), dp[i][2] = v[i][2] + max(dp[i - 1][1], dp[i - 1][3]), dp[i][3] = v[i][3] + max(dp[i - 1][1], dp[i - 1][2]);
    }
